...
try:
    ...
    print "Warning: falling back to non-accellerated AGC"

try:
    from mig_fk import fkmig
except ImportError:
    print "Warning: F-K migration module not loaded"

try:
    import pywavelet
except ImportError:
    print "Warning: pywavelet module not loaded"
...
class Gather:
    ...
    def _getkernel(self, width, kind):
        ...
        if kind == 'boxcar':
            ...
            sys.stderr.write("filter kind not recognized: {0}".format(
                            kind))
    ...
    def LoadPicks(self, infile):
        ...
        try:
            ...
            retained_dc = []
            retained_bed = []
            #for i, p in enumerate(zip(dc_points,bed_points)):
                #loc = 'location_{0}'.format(i)
                #if self.retain[loc]:
                    #retained_dc.append(p[0])
                    #retained_bed.append(p[1])
            #self.dc_picks = np.array(retained_dc)
            #self.bed_picks = np.array(retained_bed)
